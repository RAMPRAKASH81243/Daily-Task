...
import com.ram.employeeapi.service.PatternMatcherService;
...
public class EmployeeController {
	...
	@Autowired
	private PatternMatcherService matcherService;
	...
	public ResponseEntity<EmployeeResponse> cloneEmployee(@Valid @RequestBody CloneEmployeeRequest request,
		...
		EmployeeResponse response = employeeService.cloneEmployee(request, clonedBy);
		return new ResponseEntity<>(response, HttpStatus.CREATED);
	}

	@PostMapping("/regex")
	public ResponseEntity<Boolean> patternMatcher(@Valid @RequestBody PatternRequest request) {
		boolean response = matcherService.matchPattern(request);
